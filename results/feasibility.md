...
| F6 | Episode wall time | ≤ 20 s closed-loop w/ ~19 renders | 150 steps @10 Hz, 19 obs, random policy, Franka + tube + rack, Cycles/Metal 32 spp: **7.8 s total** (Blender 357 ms/obs, MuJoCo mask 15 ms/obs, dummy Model-A inference 34 ms/obs, sim 0.05 s). Bridge setup 0.3 s once. `scripts/bench_episode.py`, `render/bridge.py` | **PASS** | 5,400 episodes ≈ 12 h single process. Blender is ~87% of episode time; EEVEE tier would cut episodes to ~2.5 s. Re-measured after fixing robot mesh placement (build from `model.mesh_vert`, not raw OBJ): **8.24 s** total, Blender 381 ms/obs. Pose sync verified against MuJoCo render: `results/week0/episode/pose_check_side_by_side.png`. |

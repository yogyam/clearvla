...
DEFAULT = dict(lr=1e-4, lr_min=1e-5, warmup=1000, wd=0.01, batch=64, steps=50000, clip=1.0, ema=0.999, amp="fp16",
...
def main():
    ...
    train = FrameDataset("train", n_val=cfg["n_val"], limit=a.overfit or None)
    val = FrameDataset("val", n_val=cfg["n_val"], norm=train.norm) if not a.overfit else train
    ...
    model = ClearVLA().to(dev); ema = copy.deepcopy(model).eval()

...
class FrameDataset(Dataset):
    def __init__(self, split="train", n_val=15, source="rgb_blender", feat_dir="datasets/features", raw_dir="datasets/raw",
                 tasks=TASKS, materials=MATERIALS, norm=None, limit=None):
        ...
        for t in tasks:
            ...
            for m in materials:
                key = f"{t}_{m}_{source}"; idx = pd.read_parquet(self.feat / f"{key}.index.parquet")
                self.cells[(t, m)] = dict(path=self.feat / f"{key}.fp16.memmap", n=len(idx), mm=None)
    ...
    def __getitem__(self, i):
        ...
        vis = torch.from_numpy(np.asarray(self._mm(t, m)[row]))                                   # (196,768) fp16
